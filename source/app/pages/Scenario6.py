...
from plots import (create_cumulative_rewards_animation, create_cumulative_rewards_animation_6)
from app.pages.ScenarioBase import ExperimentBase


class ScenarioF(ExperimentBase):
...
def SF_main(arms_range, n_arm, models_choice, retrain_mode):
    st.info("In this simulation, as it's a long run, we will fix training data to 2000,\
            ...
            ,icon="📃")
    scenarioF = ScenarioF(3, train_model_6)
    scenarioF.train_and_display_results(
    ...
    with st.container():
        scenarioF.display_results(st.session_state, create_cumulative_rewards_animation_6)

    scenarioF.display_results_changable(st.session_state)
