...
import os
import mlflow
from utils import ( prepare_customer_data)
from train.train import (train_model)
from plots import (create_cumulative_rewards_animation, plot_customer_reward_distribution)
from app.pages.ScenarioBase import ExperimentBase



class ScenarioD(ExperimentBase):
        ...
        with st.container():
            ...
            with tab1:
                if 'result_retrain_every_data' not in st.session_state:
                    ...
                    if "prepared_results" not in st.session_state:
                        st.session_state.prepared_results = prepare_customer_data(st.session_state.result_retrain_every_data)
                    ...
                    fig = plot_customer_reward_distribution(
            ...
            with tab2:
                ...
                if 'result_retrain_every_2_days' not in st.session_state:
                    ...
                    if "prepared_results" not in st.session_state:
                        st.session_state.prepared_results = prepare_customer_data(st.session_state.result_retrain_every_2_days)
...
def SD_main(n_data, arms_range, n_arm, models_choice, features_choice, retrain_mode):
    scenarioD = ScenarioD(3, train_model)
    scenarioD.train_and_display_results(
        n_data = n_data,
    ...
    with st.container():
        scenarioD.display_results(st.session_state, create_cumulative_rewards_animation)

    scenarioD.display_results_changable(st.session_state)
